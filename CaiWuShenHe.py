...
class CaiWuShenHe():
    ...
    def CaiWuShenHe(self):
        ...
        taskId1 = getsql.sql.CaiWuSheHe()[1]
        a = rdexcel.readexcel()
        ...
        for i in range(0, len(taskId1)):
            ...
            print ("主贷人:" + customer + "  财务审核领取:" + response.text1["header"]["msg"][0])
	...
	"header": {
		...
		"token": token
            ...
            url4 = api.API.SetUrl() + "work/handle/auditFinance"
            data = json.dumps(payload2)
            response = requests.request("POST", url4, data=data, headers=headers)
            response.text1 = json.loads(response.text)
            customer = getsql.sql.GetName()[i]
            # print(response.text)
            print ("主贷人:" + customer + "  财务审核:" + response.text1["header"]["msg"][0])
# a=CaiWuShenHe.CaiWuShenHe()