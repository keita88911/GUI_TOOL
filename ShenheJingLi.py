...
class ShenheJingLi():
    @classmethod
    def ShenheJL(self):
        token = weblogin.gettoken.JingLintoken()
        ...
        for i in range(0, len(listdata1)):
            if int(listdata1[i]['loanAmount']) >= 50000:
                ...
                print ("主贷人:" + customer + "  经理领取:" + response.text1["header"]["msg"][0])
                payload2 = {
                    "data": {
                        "lowLoan": "0",
                        "isSeparate": "1",
                ...
                url4 = api.API.SetUrl() + "work/handle/managerAudit"
                data = json.dumps(payload2)
                response = requests.request("POST", url4, data=data, headers=headers)
                response.text1 = json.loads(response.text)
                customer = getsql.sql.GetName()[i]
                print ("主贷人:" + customer + "  经理审核:" + response.text1["header"]["msg"][0])
# a=ShenheJingLi.ShenheJL()
